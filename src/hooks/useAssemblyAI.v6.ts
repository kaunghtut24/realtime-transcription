...
const WS_API_BASE_URL = 'wss://streaming.assemblyai.com/v3';
...
export function useAssemblyAI({ apiKey, onTurn }: UseAssemblyAIProps) {
  ...
  const handleMessage = useCallback((data: any) => {
    try {
      ...
      if (message.type === 'FinalTranscript' || message.message_type === 'FinalTranscript') {
        onTurn({
          type: 'Turn',
      ...
      } else if (message.message_type === 'SessionBegins' || message.type === 'SessionBegins') {
        sessionIdRef.current = message.session_id;
        console.log('🎙️ Session started:', message.session_id);
      } else if (message.message_type === 'SessionTerminated' || message.type === 'SessionTerminated') {
