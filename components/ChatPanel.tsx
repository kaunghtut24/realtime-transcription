...
export const ChatPanel: React.FC<ChatPanelProps> = ({ history, isSending, onSendMessage }) => {
  ...
  return (
    <div className="mt-4 bg-gray-800/50 rounded-lg shadow-lg flex flex-col h-full max-h-[40vh]">
        ...
        <div ref={scrollRef} className="flex-grow p-4 overflow-y-auto space-y-4">
            {history.map((chat, index) => (
                <div key={index} className={`flex items-start gap-3 ${chat.role === 'user' ? 'justify-end' : ''}`}>
                    ...
                    <div className={`text-sm p-3 rounded-lg max-w-sm ${chat.role === 'model' ? 'bg-gray-700 text-gray-200' : 'bg-brand-blue text-white'}`}>
                        {chat.text}
                    </div>
